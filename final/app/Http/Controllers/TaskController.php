...
class TaskController extends Controller
{
    ...
    {
        // Get all departments with their users count and first few users
        $departments = Department::withCount(['users as users_count'])
            ->with([
                'users' => function ($query) {
                    $query->withCount(['tasks as tasks_count'])
                        ->with('role')
                        ->limit(3); // Show only 3 members initially
                }
            ])
            ->orderBy('name')
            ->get();

        // Keep your existing dashboard stats if needed
        $dashboardStats = [
            'totalTasks' => Task::count(),
            'totalMembers' => User::count(),
            'inProgressTasks' => Task::whereHas('users', function ($query) {
